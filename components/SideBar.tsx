...
const useCollectionData = <T,>(query: any): UseCollectionDataReturnType<T> => {
  ...
  useEffect(() => {
    ...
    if (query) {
      unsubscribe = onSnapshot(query, (snapshot: QuerySnapshot) => {
...
export default function PersistentDrawerLeft() {
  ...
  const selectedBotRef = useRef<string | null>("AttyChat");
  const [currentBot, setCurrentBot] = useState<Bot | null>(null);

  useEffect(() => {
    if (!pathname || !selectedBotRef.current) return;
console.log("pathname", pathname);
  ...
  }, [pathname, selectedBotRef.current, bots]);
